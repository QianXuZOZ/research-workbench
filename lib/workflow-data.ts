...
export function getFocusData() {
  ...
  const todayTasks=sqlite.prepare(`SELECT * FROM tasks WHERE archived_at IS NULL AND status!='done'
    ...
    CASE priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END,due_at LIMIT 20`).all(today,today) as Row[];
  const weekTasks=sqlite.prepare(`SELECT * FROM tasks WHERE archived_at IS NULL AND status!='done' AND due_at>? AND due_at<=?
    ORDER BY due_at,CASE priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END LIMIT 30`).all(today,weekEnd) as Row[];
  const inbox=sqlite.prepare("SELECT * FROM inbox_items WHERE archived_at IS NULL AND status='inbox' ORDER BY created_at DESC LIMIT 8").all() as Row[];
  ...
  const staleProjects=sqlite.prepare(`SELECT id,title,status,progress,risk,updated_at FROM projects WHERE archived_at IS NULL
    AND status IN ('planning','active','paused') AND substr(updated_at,1,10)<? ORDER BY updated_at LIMIT 8`).all(staleBefore) as Row[];
...
export function getWeeklyReviewData() {
  ...
  const recentFindings=sqlite.prepare("SELECT id,title,status,confidence,created_at FROM findings WHERE archived_at IS NULL AND substr(created_at,1,10) BETWEEN ? AND ? ORDER BY created_at DESC LIMIT 8").all(start,end) as Row[];
  const completedTasks=sqlite.prepare("SELECT id,title,completed_at,priority FROM tasks WHERE archived_at IS NULL AND status='done' AND substr(completed_at,1,10) BETWEEN ? AND ? ORDER BY completed_at DESC LIMIT 12").all(start,end) as Row[];
  const activity=sqlite.prepare("SELECT * FROM activity_logs WHERE substr(created_at,1,10) BETWEEN ? AND ? ORDER BY created_at DESC LIMIT 20").all(start,end) as Row[];
  const review=(sqlite.prepare("SELECT * FROM weekly_reviews WHERE period_start=? LIMIT 1").get(start) as Row | undefined) ?? null;
  const staleBefore=addDays(today,-14);
  const staleProjects=sqlite.prepare("SELECT id,title,progress,updated_at FROM projects WHERE archived_at IS NULL AND status IN ('planning','active','paused') AND substr(updated_at,1,10)<? ORDER BY updated_at LIMIT 8").all(staleBefore) as Row[];
