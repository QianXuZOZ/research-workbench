...
function initialForm(config: ModuleConfig) {
  const values: Record<string, string> = {};
  for (const field of config.fields) values[field.key] = field.options?.[0]?.[0] ?? (field.key === "progress" || field.key === "currentValue" ? "0" : "");
  return values;
}
