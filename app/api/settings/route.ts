...
const defaults = { workbenchName: "电研工作台", displayName: "研究者", institution: "", researchDirection: "电力系统", timezone: process.env.APP_TIMEZONE ?? "Asia/Hong_Kong", themePreset: "codex", themeMode: "system", themeOverrides: "{}" };
...
export async function PATCH(request: NextRequest) {
  ...
  const schema = z.object({ workbenchName: z.string().trim().min(1).max(80), displayName: z.string().trim().max(100), institution: z.string().trim().max(200), researchDirection: z.string().trim().max(200), timezone: z.string().trim().max(100), themePreset: z.enum(["codex","github","gruvbox","catppuccin","everforest","linear","notion","one","nord","tokyo-night","solarized"]), themeMode: z.enum(["light","dark","system"]), themeOverrides: z.string().max(3000) }).partial().refine((value) => Object.keys(value).length > 0, "至少需要一个设置项");
