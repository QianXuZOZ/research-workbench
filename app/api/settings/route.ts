...
const defaults = { displayName: "研究者", institution: "", researchDirection: "电力系统", timezone: process.env.APP_TIMEZONE ?? "Asia/Hong_Kong" };
...
export async function PATCH(request: NextRequest) {
  ...
  const schema = z.object({ displayName: z.string().trim().max(100), institution: z.string().trim().max(200), researchDirection: z.string().trim().max(200), timezone: z.string().trim().max(100) });
