...
export async function POST(request: NextRequest) {
  ...
  const existing = sqlite.prepare("SELECT id,title,year,doi FROM literature_items WHERE archived_at IS NULL").all() as { id: string; title: string; year: number | null; doi: string | null }[];
